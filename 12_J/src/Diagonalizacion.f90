...
! the energy spectrum Ek as a function of gammax
 program Entropy
   use MathTools
   implicit none
   double precision x,gmx(70)
   integer :: i,ii,n,j,Ite,states(6) !bb, pairs(4)
   ...
   character(len=25) :: file2
   character(len=8):: gamma
   character(len=10):: valores
   character(len=23):: string1
   character(len=27)::string2
   character(len=18) :: string3

   gamma = 'gmx7.dat'
   valores = 'Evals7.dat'
   print*, gamma//' is processing.'
   string1 =  '200J/340_N/gmx/'//gamma
   string2 = '200J/340_N/evals/'//valores
   string3 = '200J/340_N/states/'
   !bb = 7
   states = (/134,135,134,135,134,135/)
   !states = pairs((2*bb-1):(2*bb))
   ...
   open(10, file = string1, status = 'old')
   do i=1, size(gmx)
      read(10,*) x
      gmx(i) = x
   end do
   ...
   do ii=1,2 ! Number of states
      write (file2, "(A18,I3,A4)") string3,states(ii),".dat"
      open(40, file = file2, status='old')
      open(30, file = string2, status = 'old')
      ...
      do i=1, size(gmx) ! Number of lines in gmx.dat
         ...
         write(20,120) -gmx(i),evls1(states(ii))/j,Wint
         !print 120, -gmx(i),evls1(states(ii))/j,Wint
...
120 format(F18.14,4x,F19.14,4x,F22.20)
...
100 format('State: ',i4)
 ...
 subroutine Husimi(j,r,m,theta,phi,Ek,Hus,P,Q)
   ...
   double precision :: Ek(m)
   real(kind=p20) :: Hus, P, Q, theta, phi, h,b
